...
for unit_num in building_unit_types:
    menu_mp = 0
    input_mp = "baseline"
    data_folder_file_path = os.path.join("..","resstock-3.2.0_repo","resstock-3.2.0",f"minneapolis_upgrades_single_{unit_num}_unit_residence")
    print(data_folder_file_path)
    output_filepath = os.path.join("output_results",f"minneapolis_upgrades_single_{unit_num}_unit_residence", "tare_output.csv")

    # Load the annual metadata associated with this case. This will be loading my custom data format from my own ResStock runs
    df_resstock_run_am = TARE_IO.load_resstock_run_annual_metadata(data_folder_file_path, menu_mp)
    ...
    # Load the annual metadata associated with this case. This will be loading my custom data format from my own ResStock runs
    try:
        df_resstock_run_am_mp8 = TARE_IO.load_resstock_run_annual_metadata(data_folder_file_path, menu_mp, baseline_data=df_resstock_run_am)
    except Exception as e:
        print("This does not apply to this building", e)
        continue
    ...
    for policy_scenario in policy_scenarios:
        df_resstock_run_am_mp8 = base_TARE.calculate_public_npv(df_resstock_run_am_mp8, df_resstock_run_am_mp8, menu_mp, policy_scenario, interest_rate=0.02)

    for policy_scenario in policy_scenarios:
        df_resstock_run_am_mp8 = base_TARE.calculate_private_NPV(df_resstock_run_am_mp8, df_resstock_run_am_mp8, 0.02, input_mp, menu_mp, policy_scenario)

    for policy_scenario in policy_scenarios:
        df_resstock_run_am_mp8 = base_TARE.adoption_decision(df_resstock_run_am_mp8, policy_scenario, menu_mp)
