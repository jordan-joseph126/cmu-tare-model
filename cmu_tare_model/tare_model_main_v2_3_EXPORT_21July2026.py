...
# %%
if GRID_IMPACT_ANALYSIS:
    # Manuscript peak-demand figure: rows = technology (top MP3, bottom MP4);
    # columns = subsidy scenario (left Unsubsidized, right June 2026 without fuel
    # switching). BOTH columns use the CONSTRAINED economic-adopter set (NPV >= 0)
    # under the heating + cooling replacement-credit scope (heatingLCC_coolingLCC);
    # the two columns differ only by which rebate regime set the NPV. The baseline
    # profile is drawn in every panel, and each panel is annotated with its own
    # baseline/scenario peak magnitude and hour.
    #
    # This cell derives its two constrained adopter sets directly from the per-MP
    # TARE frames, so it does not depend on adopter_ids_by_mp (which carries only
    # the default heatingLCC_coolingSavings_sub scope). peak_load_functions.py is
    # reused without changes.
    from cmu_tare_model.grid_impact.peak_load_functions import gisjoin_to_fips

    # Left column = unsubsidized; right column = June 2026 rebate (no fuel
    # switching). Both under constrained NPV >= 0 adoption, heatingLCC_coolingLCC.
    subsidy_scenarios = [
        ("unsub", "heatingLCC_coolingLCC_unsub", "Unsubsidized"),
        ("sub_june2026", "heatingLCC_coolingLCC_sub_june2026",
         "2026 Rebate w/o fuel switching"),
    ]
    mp_labels = {
    ...
    # ---------- Step 7: constrained adopter IDs + county profiles ----------
    # For each (MP, subsidy scenario) collect the Allegheny bldg_ids whose
    # economic-adopter flag is 1.0 (NPV >= 0), then compute the hourly county
    # profile against the same baseline timeseries.
    peak_results_allegheny_by_mp = {}
    df_profiles_by_mp = {}

    for mp in selected_mps:
        print(f"\nComputing Allegheny county profiles for MP{mp}...")
        df_tare = DATAFRAMES_BY_MP[mp][discount_rate]
        county_fips = df_tare["county"].apply(gisjoin_to_fips)
        allegheny_mask = county_fips == TEST_FIPS

        peak_results_allegheny_by_mp[mp] = {}
        df_profiles_by_mp[mp] = {}

        for scenario_key, npv_case, scenario_label in subsidy_scenarios:
            # Pass the scope explicitly; the module default is a different scope
            # (heatingLCC_coolingSavings_sub). The cost-scenario token is ignored
            # when building the column name.
            adopter_col = find_adoption_column(
                df_tare, mp, REMDB_COST_SCENARIO_KEYS[0], npv_case=npv_case,
            )
            is_adopter = df_tare[adopter_col] == 1.0
            constrained_ids = df_tare.index[allegheny_mask & is_adopter].tolist()

            df_profile, peak_result = compute_county_scenario_profile(
                df_ts_baseline_allegheny,
                df_ts_upgrade_allegheny_by_mp[mp],
                adopter_bldg_ids=constrained_ids,
            )
            peak_results_allegheny_by_mp[mp][scenario_key] = peak_result
            df_profiles_by_mp[mp][scenario_key] = df_profile

            print(f"  [{scenario_label}] adopters: {peak_result['n_adopters']:,d}"
                  f" / {peak_result['n_total_buildings']:,d}")
            print(f"    baseline peak : {peak_result['baseline_peak_mw']:.2f} MW"
                  f" @ hour {peak_result['peak_hour_baseline']}")
            print(f"    scenario peak : {peak_result['scenario_peak_mw']:.2f} MW"
                  f" @ hour {peak_result['peak_hour_scenario']}")
            print(f"    delta         : {peak_result['delta_mw']:+.2f} MW")
            assert len(df_profile) == 8760

    print("\n[OK] Step 7 PASSED -- profiles for "
          f"{list(peak_results_allegheny_by_mp.keys())} x "
          f"{[s[0] for s in subsidy_scenarios]}")

    # ---------- Visualization: 2x2, white background (scoped locally) ----------
    # Scope the white style to THIS figure only via sns.axes_style; the global
    # sns.set_theme(style='darkgrid') that colors every other manuscript figure
    # is left untouched.
    with sns.axes_style("white"):
        fig, axes = plt.subplots(2, 2, figsize=(16, 10), sharey=False)
        fig.patch.set_facecolor("white")

        # Rows = technology (MP3 top, MP4 bottom); columns = subsidy scenario.
        for row_idx, mp in enumerate(selected_mps):
            for col_idx, (scenario_key, _npv, scenario_label) in enumerate(
                    subsidy_scenarios):
                ax = axes[row_idx, col_idx]
                ax.set_facecolor("white")
                df_profile = df_profiles_by_mp[mp][scenario_key]
                peak_result = peak_results_allegheny_by_mp[mp][scenario_key]
    ...
    if SAVE_FIGURES:
        out_path = os.path.join(
            PROJECT_ROOT,
            "outputs",
            f"allegheny_demand_2x2_MP{'_'.join(str(m) for m in selected_mps)}.png",
        )
...
# gdf_counties_raw (same county shapefile loaded via COUNTY_SHAPEFILE_PATH); it
# does not re-read the shapefile.
if gdf_counties_raw is not None:
    # Census county columns: GEOID = 5-digit FIPS, NAME = county name,
    # STATEFP = 2-digit state FIPS (used here as the state key).
    CENSUS_FIPS_COL = "GEOID"
    CENSUS_NAME_COL = "NAME"
    CENSUS_STATEFP_COL = "STATEFP"

    _expected_cols = {CENSUS_FIPS_COL, CENSUS_NAME_COL, CENSUS_STATEFP_COL}
    _missing = _expected_cols - set(gdf_counties_raw.columns)
    if _missing:
        raise KeyError(
            f"County shapefile missing expected columns: {_missing}. "
    ...
    county_geo_df = gdf_counties_raw[
        [CENSUS_FIPS_COL, CENSUS_NAME_COL, CENSUS_STATEFP_COL]
    ].rename(
        columns={
            CENSUS_FIPS_COL: "fips_5digit",
            CENSUS_NAME_COL: "county_name",
            CENSUS_STATEFP_COL: "state_fips",
        }
    ).copy()
