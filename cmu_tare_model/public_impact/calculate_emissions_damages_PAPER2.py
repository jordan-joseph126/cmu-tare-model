...
from cmu_tare_model.public_impact.create_lookup_emissions_fossil_fuel import lookup_emis_fossil_fuel
...
#     if df_copy.empty:
#         raise ValueError("Input DataFrame is empty. A non-empty DataFrame is required for emissions and damages calculations.")
...
def calculate_damages_grid_scenario(df_copy, df_baseline_damages_copy, df_detailed_damages, menu_mp, td_losses_multiplier, lookup_co2e_emis_electricity, cambium_scenario, scenario_prefix, 
                                    hdd_factors_df, lookup_emis_fossil_fuel, lookup_health_damages_electricity, EPA_SCC_USD2023_PER_TON, equipment_specs):
    """
    Calculate lifetime and annual damages for a given electricity grid scenario.

    This function computes annual and lifetime emissions and damages for each equipment category,
    then aggregates and concatenates these results to the detailed damages DataFrame. Avoids repeated
    DataFrame insertions by collecting results in dictionaries.

    Args:
        df_copy (pandas.DataFrame): Input DataFrame with emissions and consumption data.
        df_baseline_damages_copy (pandas.DataFrame): Baseline damages DataFrame; used when menu_mp != 0.
        df_detailed_damages (pandas.DataFrame): DataFrame to store aggregated annual damage results.
        menu_mp (int): Measure package identifier.
        td_losses_multiplier (float): Multiplier for transmission and distribution losses.
        lookup_co2e_emis_electricity (dict): Lookup table for CO2e electricity emissions factors.
        cambium_scenario (str): Scenario label used for lookup keys.
        scenario_prefix (str): Prefix string for naming output columns.
        hdd_factors_df (dict): Mapping of year labels to HDD factors (as pandas.Series) by region.
        lookup_emis_fossil_fuel (dict): Lookup table for fossil fuel emissions factors.
        lookup_health_damages_electricity (dict): Lookup table for electricity health damage factors.
        EPA_SCC_USD2023_PER_TON (float): Social Cost of Carbon (SCC) in USD per ton (adjusted for inflation).
        equipment_specs (dict): Dictionary mapping equipment categories (str) to their lifetimes (int).

    Returns:
        tuple: A tuple containing:
            - df_new_columns (pandas.DataFrame): DataFrame of computed lifetime aggregated columns.
            - df_detailed_damages (pandas.DataFrame): Updated DataFrame with concatenated annual damage details.

    Raises:
        ValueError: If the input DataFrame (df_copy) is empty.
    """
    ...
    for category, lifetime in equipment_specs.items():
        ...
        # Initialize lifetime accumulators for climate emissions, climate damages, and health damages.
        lifetime_climate_emissions = {'lrmer': pd.Series(0.0, index=df_copy.index),
                                      'srmer': pd.Series(0.0, index=df_copy.index)}
        lifetime_climate_damages = {'lrmer': pd.Series(0.0, index=df_copy.index),
                                    'srmer': pd.Series(0.0, index=df_copy.index)}
        ...
        for year in range(1, lifetime + 1):
            year_label = year + 2023  # Compute the actual year label based on lifetime offset

            # Retrieve HDD factor for the current year; use default factor if not found.
            hdd_factor = hdd_factors_df.get(year_label, pd.Series(1.0, index=df_copy.index))
            adjusted_hdd_factor = hdd_factor if category in ['heating', 'waterHeating'] else pd.Series(1.0, index=df_copy.index)
            ...
            # Calculate annual climate emissions and damages.
            climate_results, annual_climate_emissions, annual_climate_damages = calculate_climate_emissions_and_damages(
                df=df_copy, category=category, year_label=year_label, adjusted_hdd_factor=adjusted_hdd_factor, td_losses_multiplier=td_losses_multiplier,
                lookup_co2e_emis_electricity=lookup_co2e_emis_electricity, cambium_scenario=cambium_scenario, EPA_SCC_USD2023_PER_TON=EPA_SCC_USD2023_PER_TON,
                total_fossil_emissions=total_fossil_emissions, scenario_prefix=scenario_prefix, menu_mp=menu_mp
            ...
            # Calculate annual health damages.
            health_results, annual_health_damages = calculate_health_damages(
                df=df_copy, category=category, year_label=year_label, adjusted_hdd_factor=adjusted_hdd_factor, td_losses_multiplier=td_losses_multiplier,
                lookup_health_damages_electricity=lookup_health_damages_electricity, cambium_scenario=cambium_scenario, 
                total_fossil_emissions=total_fossil_emissions, scenario_prefix=scenario_prefix, POLLUTANTS=POLLUTANTS, menu_mp=menu_mp
            ...
            # Update lifetime accumulators with annual results.
            for mer_type in ['lrmer', 'srmer']:
                lifetime_climate_emissions[mer_type] += annual_climate_emissions.get(mer_type, 0.0)
                lifetime_climate_damages[mer_type] += annual_climate_damages.get(mer_type, 0.0)
            ...
            # Concatenate annual results into the detailed damages DataFrame.
            annual_data_all = {**climate_results, **health_results}
            if annual_data_all:
                df_detailed_damages = pd.concat([df_detailed_damages, pd.DataFrame(annual_data_all, index=df_copy.index)], axis=1)

        # After computing all years for this category, create a dictionary for lifetime values.
        lifetime_dict = {}
        for mer_type in ['lrmer', 'srmer']:
            # Define column names for lifetime emissions and damages.
            lifetime_emissions_col = f'{scenario_prefix}{category}_lifetime_tons_co2e_{mer_type}'
            lifetime_damages_col = f'{scenario_prefix}{category}_lifetime_damages_climate_{mer_type}'
        ...
        # Compute avoided columns only if all required baseline columns are present.
        if menu_mp != 0 and df_baseline_damages_copy is not None:
            required_baseline = [
                f'baseline_{category}_lifetime_tons_co2e_lrmer',
                f'baseline_{category}_lifetime_tons_co2e_srmer',
                f'baseline_{category}_lifetime_damages_climate_lrmer',
                f'baseline_{category}_lifetime_damages_climate_srmer',
                f'baseline_{category}_lifetime_damages_health'
            ]
            if all(col in df_baseline_damages_copy.columns for col in required_baseline):
                for mer_type in ['lrmer', 'srmer']:
                    avoided_emissions_col = f'{scenario_prefix}{category}_avoided_tons_co2e_{mer_type}'
                    avoided_damages_col = f'{scenario_prefix}{category}_avoided_damages_climate_{mer_type}'
                    ...
                    lifetime_dict[avoided_emissions_col] = np.round(
                        df_baseline_damages_copy[baseline_emissions_col] - lifetime_dict[f'{scenario_prefix}{category}_lifetime_tons_co2e_{mer_type}'], 2
                    )
                    lifetime_dict[avoided_damages_col] = np.round(
                        df_baseline_damages_copy[baseline_damages_col] - lifetime_dict[f'{scenario_prefix}{category}_lifetime_damages_climate_{mer_type}'], 2
                    )
                avoided_health_damages_col = f'{scenario_prefix}{category}_avoided_damages_health'
                baseline_health_col = f'baseline_{category}_lifetime_damages_health'
                lifetime_dict[avoided_health_damages_col] = np.round(
                    df_baseline_damages_copy[baseline_health_col] - lifetime_health_damages.round(2), 2
                )
            else:
                print(f"Warning: Missing baseline columns for {category}. Avoided values skipped.")

        # Always add lifetime health damages column.
        lifetime_health_damages_col = f'{scenario_prefix}{category}_lifetime_damages_health'
        lifetime_dict[lifetime_health_damages_col] = lifetime_health_damages.round(2)

        # Merge all lifetime columns into the aggregated new columns dictionary.
        new_columns_data.update(lifetime_dict)

        # Concatenate lifetime results for this category to the detailed damages DataFrame.
        df_detailed_damages = pd.concat([df_detailed_damages, pd.DataFrame(lifetime_dict, index=df_copy.index)], axis=1)

    # Finalize the DataFrame for lifetime columns.
    df_new_columns = pd.DataFrame(new_columns_data, index=df_copy.index)

    # Return the new lifetime columns DataFrame and the updated detailed damages DataFrame.
    return df_new_columns, df_detailed_damages
