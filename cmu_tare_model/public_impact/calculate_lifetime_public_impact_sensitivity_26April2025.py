...
def calculate_public_npv(
    ...
    if all_new_dfs:
        ...
        # Add temporary validation columns to df_combined_new for masking
        ...
        # Apply final masking
        df_combined_new = apply_final_masking(df_combined_new, all_columns_to_mask, verbose=False)
        ...
        if validation_columns:
            df_combined_new = df_combined_new.drop(columns=validation_columns)
        ...
        for scc in SCC_ASSUMPTIONS:
            ...
            for year in range(1, lifetime + 1):
                ...
                if climate_cols_exist:
                    ...
                    # ============================================================================
                    # UPDATE: Change NPV accumulation approach for climate damages
                    # Store in list instead of updating NPV incrementally
                    # Apply validation mask for measure packages
                    if menu_mp != "0":
                        avoided_climate.loc[~valid_mask] = 0.0
                ...
                if health_cols_exist:
                    ...
                    # ============================================================================
                    # UPDATE: Change NPV accumulation approach for health damages
                    # Store in list instead of updating NPV incrementally
                    # Apply validation mask for measure packages
                    if menu_mp != "0":
                        avoided_health.loc[~valid_mask] = 0.0
            ...
            # ============================================================================
            # UPDATE: Add summing logic: Sum up all yearly avoided damages using pandas operations
