...
# %%
# # --- Hot-reload the font-size constants + geospatial module (no kernel restart) ---
# # The map module imports the MAP_*_FONT_SIZE constants by name at import time, so
# # we must reload constants FIRST, then the module that consumed them, then re-bind
# # the plotting function into this namespace.
# import importlib
# import cmu_tare_model.constants
# import cmu_tare_model.adoption_kpis.visualize_geospatial_data as _viz

# importlib.reload(cmu_tare_model.constants)
# importlib.reload(_viz)

# # Re-bind the name the choropleth cell calls so it uses the reloaded version
# plot_combined_choropleth = _viz.plot_combined_choropleth

# print(
#     f"[OK] Reloaded. cbar label={cmu_tare_model.constants.MAP_CBAR_LABEL_FONT_SIZE}, "
#     f"cbar ticks={cmu_tare_model.constants.MAP_CBAR_TICK_LABEL_SIZE}, "
#     f"title={cmu_tare_model.constants.MAP_TITLE_FONT_SIZE}"
# )
...
if GRID_IMPACT_ANALYSIS: 
    ...
    with sns.axes_style("white"):
        # sharey='row' -> the two panels in each row share one y-scale; rows stay
        # independent so each scenario keeps its own MW range.
...
# %%
import cmu_tare_model.utils.column_names as column_names
importlib.reload(column_names)
