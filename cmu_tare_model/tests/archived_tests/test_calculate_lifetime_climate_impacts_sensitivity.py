...
def sample_homes_df():
    ...
    This fixture creates test data with:
    - Various regions and county identifiers for lookups
    - Valid and invalid homes across different equipment categories
    - Different fuel types and technologies
    - Consumption data for multiple years
    ...
    for category in ['heating', 'waterHeating', 'clothesDrying', 'cooking']:
        for year in range(2024, 2030):  # Multiple years for lifetime calculations
...
def dummy_define_scenario_settings(monkeypatch, dummy_scc_lookup):
    ...
    def dummy(menu_mp, policy_scenario):
        ...
        # Create lookup dictionary with a fallback mechanism for any region
        dummy_lookup_emissions_electricity_climate = {}
        
        # Add standard test regions from sample_homes_df
        ...
        # Add special test region for boundary lifetime test
        dummy_lookup_emissions_electricity_climate[(cambium_scenario, "test_cat")] = dummy_emission_factors_by_year
        
        # IMPROVEMENT: Add fallback handler for any unexpected region
        dummy_lookup_emissions_electricity_climate[(cambium_scenario, "UnmappedRegion")] = dummy_emission_factors_by_year
...
def mock_precompute_hdd_factors(monkeypatch):
    ...
    def mock_function(df):
        ...
        # IMPROVEMENT: Create HDD factors for a wider range of years (2023-2040)
        # This ensures we have factors for any year the code might request
        hdd_factors = {}
        for year in range(2023, 2041):  # Expanded range
...
def test_calculate_climate_emissions_and_damages_success(
    ...
    # Verify annual emissions and damages structures
    for mer_type in MER_TYPES:
        assert mer_type in annual_emissions, f"Missing {mer_type} in annual_emissions"
        
        for scc_assumption in SCC_ASSUMPTIONS:
            key = (mer_type, scc_assumption)
            assert key in annual_damages, f"Missing key {key} in annual_damages"
...
def test_calculate_climate_emissions_damages_calculation_correctness(
    ...
    This test verifies that emissions and damages are correctly calculated
    based on input consumption and emission factors.
    """
    # Setup test parameters
    category = 'heating'
    year_label = 2024
    ...
    # Get scenario parameters
    scenario_prefix, cambium_scenario, lookup_emissions_fossil_fuel, lookup_emissions_electricity_climate, _, _ = dummy_define_scenario_settings(0, "No Inflation Reduction Act")
    
    # Create controlled test data for fossil fuel emissions
    # Use a fixed value for easier verification
    ...
    # Calculate expected electricity emissions using the exact same formula as the function
    # This helps verify the calculation logic is correct
    ...
    # scc_value = lookup_climate_impact_scc["central"][year_label]
    ...
    assert abs(actual_damages - expected_damages) < 0.0001, \
        f"Damages calculation incorrect. Expected {expected_damages}, got {actual_damages}"
...
def test_calculate_lifetime_climate_impacts_missing_column(
    ...
    df_missing = df_missing.drop(columns=['census_division'])
...
def test_calculate_lifetime_climate_impacts_missing_region_factor(
    ...
    """
    # Create a modified DF with a region not in our mock lookup
    df_modified = sample_homes_df.copy()
    df_modified['gea_region'] = 'UnmappedRegion'
    
    # Call the function and expect an error about missing emission factors
    ...
    assert "Emission factor for LRMER not found" in error_msg, \
        f"Expected emission factor error, got: {error_msg}"
...
def test_calculate_lifetime_climate_impacts_boundary_lifetime(
    ...
    try:
        # Add required columns for the test category
        sample_homes_df_modified = sample_homes_df.copy()
        sample_homes_df_modified[f'include_{test_category}'] = True
        sample_homes_df_modified[f'base_electricity_{test_category}_consumption'] = 100
        sample_homes_df_modified[f'base_naturalGas_{test_category}_consumption'] = 200
        sample_homes_df_modified[f'base_propane_{test_category}_consumption'] = 300
        sample_homes_df_modified[f'base_fuelOil_{test_category}_consumption'] = 400
        
        # Override equipment specs with just the test category
        monkeypatch.setattr(
...
def test_missing_required_columns(
    ...
    # Check if the error message contains any of the missing column names
    error_msg = str(excinfo.value)
    error_contains_any_column = any(col in error_msg for col in cols_to_drop)
    assert error_contains_any_column or "column" in error_msg.lower(), \
