...
# %% [markdown]
# # Baseline Scenario
# - Uncomment Code Below For Individual scenario runs (rather than full model MP8-Basic, mp10-Moderate, MP10-Advanced)

# %%
# import os

# from config import PROJECT_ROOT
# # from cmu_tare_model.model_scenario.tare_baseline_v2 import *
...
# %% [markdown]
# ## Dataframe used for other end-uses (MP10)

# %%
# Measure Package 10
menu_mp = 10
input_mp = 'upgrade10'
scenario_name = 'Advanced-BAU'
...
filename = "upgrade10_metadata_and_annual_results.csv"
relative_path = os.path.join("cmu_tare_model", "data", "euss_data", "resstock_amy2018_release_1.1", "state", filename)
file_path = os.path.join(PROJECT_ROOT, relative_path)
...
# Fix DtypeWarning error in columns 'in.neighbors' and 'in.geometry_stories_low_rise'
columns_to_string = {11: str, 61: str, 121: str, 103: str, 128: str, 129: str}
df_euss_am_mp10 = pd.read_csv(file_path, dtype=columns_to_string, index_col="bldg_id") # UPDATE: Set index to 'bldg_id' (unique identifier)
print(f"DATAFRAME SIZE before applying any filters: {df_euss_am_mp10.shape}")

# Filter for occupied homes
occupancy_filter = df_euss_am_mp10['in.vacancy_status'] == 'Occupied'
df_euss_am_mp10 = df_euss_am_mp10.loc[occupancy_filter]
print(f"DATAFRAME SIZE after filtering for 'Occupied' homes: {df_euss_am_mp10.shape}")

# Filter for single family home building type
house_type_list = ['Single-Family Attached', 'Single-Family Detached']
house_type_filter = df_euss_am_mp10['in.geometry_building_type_recs'].isin(house_type_list)
df_euss_am_mp10 = df_euss_am_mp10.loc[house_type_filter]
print(f"DATAFRAME SIZE after filtering for 'Single-Family Attached' and 'Single-Family Detached' homes: {df_euss_am_mp10.shape}")
...
# Filter down to state or city
    ...
    print(f"You chose to filter for: {input_state}")
    state_filter = df_euss_am_mp10['in.state'].eq(input_state)
    df_euss_am_mp10 = df_euss_am_mp10.loc[state_filter]
    ...
    # Filter to a city within the selected state
        ...
        print(f"You chose to filter for: {input_state}, {input_cityFilter}")
        city_filter = df_euss_am_mp10['in.city'].eq(f"{input_state}, {input_cityFilter}")
        df_euss_am_mp10 = df_euss_am_mp10.loc[city_filter]

# Display the filtered dataframe
# Display the filtered dataframe
print(f"DATAFRAME SIZE after applying geographic filter: {df_euss_am_mp10.shape}")
print(df_euss_am_mp10)
...
====================================================================================================================================================================
You'll notice that the number of rows differs from df_euss_am_mp7 and df_euss_am_mp10.
      - df_euss_am_baseline_home has fewer rows (representative dwelling units) because a tech filter was applied. 
      - df_euss_am_mp10_home will have the same number of rows as df_euss_am_baseline_home after df_enduse_compare function is run.
...
====================================================================================================================================================================
df_euss_am_mp10_home will be created by running the df_enduse_compare function (contains post-retrofit consumption data for the entire home in 2024).
process_euss_data.py file contains the function definition.
""")

# df_enduse_compare(df_mp, category, df_baseline):
df_euss_am_mp10_home = df_enduse_compare(df_mp = df_euss_am_mp10,
                                        input_mp=input_mp,
                                        menu_mp=menu_mp,
                                        df_baseline = df_euss_am_baseline_home,
                                        df_cooking_range=df_euss_am_mp7,
                                        )
# df_euss_am_mp10_home
# print(df_euss_am_mp10_home)

# %%
# from cmu_tare_model.energy_consumption_and_metadata.project_future_energy_consumption import *

df_mp10_scenario_consumption = df_euss_am_mp10_home.copy()

# Project Future Energy Consumption
df_euss_am_mp10_home, df_mp10_scenario_consumption = project_future_consumption(df=df_euss_am_mp10_home,
...
DATAFRAME (Summary of mp10): df_euss_am_mp10_home

{df_euss_am_mp10_home}

DATAFRAME (Consumption Cols): df_mp10_scenario_consumption
      
{df_mp10_scenario_consumption}
""")
...
df_mp10_scenario_damages = df_mp10_scenario_consumption.copy()

print("\n", "Creating dataframe to store annual fuel cost calculations ...")
df_mp10_scenario_fuel_costs = df_mp10_scenario_consumption.copy()
...
df_euss_am_mp10_home, df_mp10_scenario_damages = calculate_marginal_damages(df=df_euss_am_mp10_home, menu_mp=menu_mp, policy_scenario='No Inflation Reduction Act', df_baseline_damages=df_baseline_scenario_damages, df_detailed_damages=df_mp10_scenario_damages)


print("\n","Modeling Scenario: AEO2023 Reference Case")
df_euss_am_mp10_home, df_mp10_scenario_damages = calculate_marginal_damages(df=df_euss_am_mp10_home, menu_mp=menu_mp, policy_scenario='AEO2023 Reference Case', df_baseline_damages=df_baseline_scenario_damages, df_detailed_damages=df_mp10_scenario_damages)

print(f"""  
====================================================================================================================================================================
Post-Retrofit (MP10) Marginal Damages: WHOLE-HOME
...
DATAFRAME FOR MP10 SCENARIO DAMAGES: df_mp10_scenario_damages
{df_mp10_scenario_damages}
      
SUMMARY DATAFRAME FOR MP10: df_euss_am_mp10_home
{df_euss_am_mp10_home}
====================================================================================================================================================================
""")
...
print("\n", "Modeling Scenario: No Inflation Reduction Act")
df_mp10_scenario_fuel_costs = calculate_annual_fuelCost(df=df_mp10_scenario_fuel_costs, menu_mp=menu_mp, policy_scenario='No Inflation Reduction Act', drop_fuel_cost_columns=False)

print("\n","Modeling Scenario: AEO2023 Reference Case")
df_mp10_scenario_fuel_costs = calculate_annual_fuelCost(df=df_mp10_scenario_fuel_costs, menu_mp=menu_mp, policy_scenario='AEO2023 Reference Case', drop_fuel_cost_columns=False)

print(f"""  
====================================================================================================================================================================
Post-Retrofit (MP10) Fuel Costs: WHOLE-HOME
Scenario: No Inflation Reduction Act and AEO2023 Reference Case
====================================================================================================================================================================
...
DATAFRAME FOR MP10 Fuel Costs: df_mp10_scenario_fuel_costs
{df_mp10_scenario_fuel_costs}
      ====================================================================================================================================================================
""")
...
# %% [markdown]
# ## Calculate Capital Costs (Applicatble to All Scenarios)
...
- Step 2: Calculate equipment capital costs (For space heating, include ductwork and weatherization (mp10 and MP10))
...
# %% [markdown]
# ### Space Heating with Enhanced Enclosure Upgrade (MP8 + MP2)
...
df_euss_am_mp10_home = obtain_heating_system_specs(df=df_euss_am_mp10_home)
...
df_euss_am_mp10_home = calculate_installation_cost(df=df_euss_am_mp10_home,
...
df_euss_am_mp10_home = calculate_replacement_cost(df=df_euss_am_mp10_home,
                                                 cost_dict=dict_heating_equipment_cost,
                                                 menu_mp=menu_mp,
                                                 end_use='heating')
...
df_euss_am_mp10_home = calculate_heating_installation_premium(df=df_euss_am_mp10_home,
                                                             menu_mp=menu_mp,
...
print("""
====================================================================================================================================================================
RETROFIT COSTS: Enclosure Upgrades
====================================================================================================================================================================
...
# calculate_enclosure_retrofit_upgradeCosts(df, cost_dict, retrofit_col, params_col)
df_euss_am_mp10_home = calculate_enclosure_retrofit_upgradeCosts(df=df_euss_am_mp10_home,
...
df_euss_am_mp10_home['mp9_enclosure_upgradeCost'] = round(
    df_euss_am_mp10_home['insulation_atticFloor_upgradeCost'].fillna(0.0) +
    df_euss_am_mp10_home['infiltration_reduction_upgradeCost'].fillna(0.0) +
    df_euss_am_mp10_home['duct_sealing_upgradeCost'].fillna(0.0) +
    df_euss_am_mp10_home['insulation_wall_upgradeCost'].fillna(0.0), 2
)

print(f"""  
====================================================================================================================================================================
RETROFIT CAPITAL COSTS: Enclosure Upgrades
Scenario: No Inflation Reduction Act and AEO2023 Reference Case
====================================================================================================================================================================
Measure Package 10 (MP10) contains the Basic Enclosure Upgrade (MP1). 
Cost estimates for the following upgrades were calculated:
    - Attic Floor Insulation
    - Infiltration Reduction
    - Duct Sealing
    - Drill and Fill Wall Insulation      

DATAFRAME WITH MP10 ENCLOSURE COSTS: df_euss_am_mp10_home
...
# %% [markdown]
# #### Weatherization (MP2: Enhanced Enclosure Upgrade) Capital Costs
...
# %%
print("Calculating the cost to do Enhanced Enclosure Upgrade ...")
df_euss_am_mp10_home[f'mp10_enclosure_upgradeCost'] = round(
    df_euss_am_mp10_home[f'mp9_enclosure_upgradeCost'].fillna(0.0) +
    df_euss_am_mp10_home[f'insulation_foundation_wall_upgradeCost'].fillna(0.0) +
    df_euss_am_mp10_home[f'insulation_rim_joist_upgradeCost'].fillna(0.0) +
    df_euss_am_mp10_home[f'seal_crawlspace_upgradeCost'].fillna(0.0) +
    df_euss_am_mp10_home[f'insulation_roof_upgradeCost'].fillna(0.0), 2
    )
df_euss_am_mp10_home
...
Scenario: No Inflation Reduction Act and AEO2023 Reference Case
====================================================================================================================================================================
Measure Package 10 (MP10) contains the Enhanced Enclosure Upgrade (MP2). 
Cost estimates for the following upgrades were calculated for MP2:
    - Foundation Wall Insulation
    - Rim Joist Insulation
    - Seal Crawlspace
    - Roof Insulation      

MP10 Enclosure Upgrade Costs include the Basic Enclosure Upgrade (MP1/MP10) costs PLUS the Enhanced Enclosure Upgrade (MP2) costs.
...
df_euss_am_mp10_home = df_euss_am_mp10_home.copy()

# calculate_percent_AMI(df_results_IRA, df_county_medianIncome):
df_euss_am_mp10_home = calculate_percent_AMI(df_results_IRA=df_euss_am_mp10_home)

print("Calculating rebate amounts for Space Heating ...")
df_euss_am_mp10_home = calculate_rebateIRA(df_results_IRA=df_euss_am_mp10_home,
                                          category="heating",
                                          menu_mp=menu_mp)
...
DATAFRAME: df_euss_am_mp10_home AFTER CALCULATING REBATE AMOUNTS
{df_euss_am_mp10_home}
...
# %% [markdown]
# # SCENARIO ANALYSIS: Advanced Pre-IRA Scenario
...
df_euss_am_mp10_home = calculate_public_npv(df=df_euss_am_mp10_home,
                                           df_baseline_damages=df_baseline_scenario_damages,
                                           df_mp_damages=df_mp10_scenario_damages,
                                           menu_mp=menu_mp,
...
DATAFRAME FOR MP10 AFTER CALCULATING PUBLIC NPV: df_euss_am_mp10_home
...
df_euss_am_mp10_home = calculate_private_npv(df=df_euss_am_mp10_home,
                                            df_fuel_costs=df_mp10_scenario_fuel_costs,
                                            menu_mp=menu_mp,
                                            input_mp=input_mp,
                                            policy_scenario='No Inflation Reduction Act',
                                            interest_rate=0.07,
...
DATAFRAME FOR MP10 AFTER CALCULATING PRIVATE NPV: df_euss_am_mp10_home
...
df_euss_am_mp10_home = adoption_decision(df=df_euss_am_mp10_home,
                                        menu_mp=menu_mp,
                                        policy_scenario='No Inflation Reduction Act'
                                        )
...
DATAFRAME FOR MP10 AFTER DETERMINING ADOPTION FEASIBILITY: df_euss_am_mp10_home
...
# %%
# Measure Package 10
scenario_name = 'Advanced IRA-Reference'
