...
def mask_category_specific_data(
    ...
    Updated to handle large numbers of columns efficiently using chunking.
    ...
    # Copy only the subset of columns we will modify (much smaller than copying whole df)
    sub = df.loc[:, valid_columns].copy()

    # Choose a column chunk size to limit temporary memory use.
    # If you have many thousands of columns, reduce chunk_size to 200-500.
    chunk_size = 1000
    # Split the list of valid columns into chunks
    import numpy as _np  # local alias to avoid name collisions
    col_chunks = _np.array_split(valid_columns, max(1, (len(valid_columns) + chunk_size - 1) // chunk_size))

    for chunk in col_chunks:
        cols = list(chunk)
        # Count non-NaN values before masking (operate on the small sub-DataFrame)
        non_nan_before = sub[cols].notna().sum()

        # Apply masking to the sub-DataFrame using the include flag from the original df
        sub.loc[~df[include_col], cols] = np.nan

        # Count non-NaN values after masking
        non_nan_after = sub[cols].notna().sum()

        # Handle different types safely and preserve the original try/except behavior
        diff = non_nan_before - non_nan_after
        for col in cols:
            try:
                masked_this_col = int(diff[col])
            except TypeError:
                # If we got a Series-like result, take its sum
                if isinstance(diff[col], pd.Series):
                    masked_this_col = int(diff[col].sum())
                else:
                    masked_this_col = int(float(diff[col]))

            if masked_this_col > 0 and verbose:
                print(f"    {col}: Masked {masked_this_col} values")
                masked_count += masked_this_col

    # Assign masked columns back into the original DataFrame (mutates df)
    df.loc[:, valid_columns] = sub
