...
def _map_remdb_parameters(
    ...
    params = [
        ...
        'pm1_lower_bound', 'pm1_upper_bound',  # ← ADD bounds for clamping
        'pm2_metric', 'pm2_unit', f'pm2_coef_{percentile}',
        'pm2_lower_bound', 'pm2_upper_bound',  # ← ADD bounds for clamping
...
def add_remdb_replacement_metrics(
    ...
    # Clamp metrics to REMDB valid bounds (handles out-of-bounds equipment like SEER 8 ACs)
    df_copy[pm1_col] = _clamp_metric_to_bounds(
        df_copy, pm1_col, f'{prefix}pm1_lower_bound', f'{prefix}pm1_upper_bound', 'capacity')
    df_copy[pm2_col] = _clamp_metric_to_bounds(
        df_copy, pm2_col, f'{prefix}pm2_lower_bound', f'{prefix}pm2_upper_bound', 'efficiency')
...
def add_remdb_upgrade_metrics(
    ...
    # We dont append cooling to valid_categories here because we only car about cooling replacement costs
