...
def add_remdb_replacement_metrics(
    ...
    Returns:
        DataFrame with {end_use}_replacement_metric1 and {end_use}_replacement_metric2 columns.
    ...
    if end_use == 'heating':
        ...
        df_copy[f'{end_use}_{replacement_or_upgrade}_metric2'] = pd.to_numeric(
    ...
    # ===== COOLING =====
        ...
        elif f'heating_{replacement_or_upgrade}_metric2' in df_copy.columns:
...
def map_remdb_cost_parameters(
    ...
    Args:
        df: Main DataFrame with row_id_{end_use}_{replacement_or_upgrade} column
    ...
    for param in param_cols:
        df_copy[f'{end_use}_{replacement_or_upgrade}_{param}'] = df_copy[row_id_col].map(remdb_v4_costs[param])
...
def remdb_cost_regression_formula(
    ...
    metric1 = f'{end_use}_{replacement_or_upgrade}_metric1'
    metric2 = f'{end_use}_{replacement_or_upgrade}_metric2'
    pm1_coef = f'{end_use}_{replacement_or_upgrade}_pm1_coef_{percentile}'
    pm2_coef = f'{end_use}_{replacement_or_upgrade}_pm2_coef_{percentile}'
    intercept = f'{end_use}_{replacement_or_upgrade}_intercept_{percentile}'
    multiplier_retrofit = f'{end_use}_{replacement_or_upgrade}_multiplier_retrofit'
    adder_retrofit = f'{end_use}_{replacement_or_upgrade}_adder_retrofit'
