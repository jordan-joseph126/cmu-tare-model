...
# # CORRECTED THE RSMEANS CCI LOCATION ADJUSTMENT FACTOR
...
#     """
#     Adjusts a total installed cost (2013 USD) to account for:
#       (a) Inflation from 2013 → 2023 (CPI),
#       (b) RSMeans location factor (average, since cost is total installed).

#     Expects columns:
#         hvac_cooling_type: indicates if there's existing central AC
#         heating_type: describes the heating system (Furnace, Boiler, etc.)
#         cci_loc_adjust_factor_avg: single RSMeans factor for total cost
#     """
#     df_copy = df.copy()

#     # Verify DataFrame contains necessary columns
#     necessary_columns = [
#         'hvac_cooling_type',
#         'heating_type',
#         'cci_loc_adjust_factor_avg'
#     ]
#     for col in necessary_columns:
#         if col not in df_copy.columns:
#             raise ValueError(f"DataFrame missing '{col}' column, required for cost calc.")
    
#     # Base cost is in 2013 USD --> adjust to 2023 USD
...
#         # Deetjen: Install SEER 15, 8.5 HSPF ASHP: NREL REMDB High Cost is $4800 USD-2013        
#         elif 'Boiler' in row['heating_type']:
#             premium_usd2013 = 1500

#         else:
#             raise ValueError(f"Invalid heating_type: {row['heating_type']}")

#         # (1) Inflate from 2013 → 2023 using CPI ratio
#         premium_usd2023 = premium_usd2013 * cpi_ratio_2023_2013

#         # (2) Apply the RSMeans location factor (average, total cost)
#         loc_factor = row['cci_loc_adjust_factor_avg']
#         total_cost = round(premium_usd2023 * loc_factor, 2)

#         # Store it in a new column
#         df_copy.at[index, f'mp{menu_mp}_heating_installation_premium'] = total_cost
...
# def calculate_installation_cost_per_row(df_valid, sampled_costs_dict, menu_mp, end_use, cpi_ratio_2023_2013):
#     """
#     Calculates the total installed cost for 'end_use' (e.g., heating)
#     when you have:
#       - total installed cost in 2013 USD (no separate labor/material).
#       - single RSMeans average factor in df_valid['cci_loc_adjust_factor_avg'].

#     Returns: 
#         (installation_cost, cost_column_name)
#     """

#     if 'cci_loc_adjust_factor_avg' not in df_valid.columns:
#         raise ValueError("DataFrame must have 'cci_loc_adjust_factor_avg' for location-based cost adjustment.")

#     # 1) Calculate a base cost in 2013 USD
#     #    (This example lumps all cost into one number: unitCost, otherCost, etc.)
#     base_cost_2013 = (
#         sampled_costs_dict['unitCost']
#         + sampled_costs_dict.get('otherCost', 0)  # in case 'otherCost' might not exist
#         + (df_valid['total_heating_load_kBtuh'] * sampled_costs_dict['cost_per_kBtuh'])
#     )

#     # 2) Inflate from 2013 → 2023
#     cost_2023 = base_cost_2013 * cpi_ratio_2023_2013

#     # 3) Apply the average RSMeans location factor
#     loc_factor = df_valid['cci_loc_adjust_factor_avg']
#     installation_cost = cost_2023 * loc_factor

#     # 4) Round or format as needed
#     installation_cost = installation_cost.round(2)

#     # 5) Return the series plus the column name we want
#     cost_column_name = f'mp{menu_mp}_{end_use}_installationCost'
#     return installation_cost, cost_column_name
