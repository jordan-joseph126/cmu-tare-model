...
#     df['upgrade_newInstall_HSPF'] = df['upgrade_hvac_heating_efficiency'].str.extract(r'(\d+\.\d+)')
...
def obtain_heating_system_specs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['upgrade_newInstall_HSPF'] = df['upgrade_hvac_heating_efficiency'].str.extract(r'(\d+) HSPF').astype(float)
