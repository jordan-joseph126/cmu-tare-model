...
# =================================== OLD CODE ==========================================
    ...
    base_year = 2024  # Match the default in current implementation
    for year in range(1, lifetime + 1):
        year_label = year + (base_year - 1)  # Match current implementation's formula
        annual_savings = df_fuel_costs_copy[f'{scenario_prefix}{year_label}_{category}_savings_fuelCost'].fillna(0)
        
        # Calculate discount factor using same approach as current implementation
        years_difference = year_label - base_year  # This matches how calculate_discount_factor works
        years_difference = max(0, years_difference)
        discount_factor = 1 / ((1 + interest_rate) ** years_difference)
