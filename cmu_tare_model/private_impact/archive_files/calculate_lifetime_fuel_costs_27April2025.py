...
def calculate_lifetime_fuel_costs(
    ...
    # UPDATED: Initialize a dictionary to store lifetime fuel costs columns
    lifetime_columns_data = {}

    # UPDATED: Initialize a dictionary to track columns for masking verification by category
    ...
    for category, lifetime in EQUIPMENT_SPECS.items():
        ...
        try:
            ...
            # STEP 2: Initialize result series for NPV calculations
            # This will be used as a template, not for accumulation
            lifetime_fuel_costs_template = create_retrofit_only_series(df_copy, valid_mask)
            ...
            # Create a list to store each year's annual costs
            yearly_costs_list = []
            ...
            for year in range(1, lifetime + 1):
                ...
                try:
                    ...
                    # STEP 3 & 4: Store annual costs in list instead of updating incrementally
                    # Apply validation mask to zero out invalid homes' values
                    masked_annual_costs = annual_cost_value.copy()
                    if menu_mp != 0:  # Only for measure packages
                        masked_annual_costs.loc[~valid_mask] = 0.0
                        
                    yearly_costs_list.append(masked_annual_costs)
            ...
            # Sum up all yearly costs at once using pandas operations
            if yearly_costs_list:
                # Convert list of Series to DataFrame and sum across all years
                ...
                # If no yearly costs were calculated, use the template
                lifetime_fuel_costs = lifetime_fuel_costs_template
            ...
            # Add to track columns for masking
    ...
    df_main = apply_final_masking(df_main, all_columns_to_mask, verbose=True)
    df_detailed = apply_final_masking(df_detailed, all_columns_to_mask, verbose=True)    
...
def calculate_annual_fuel_costs(
    ...
    category_columns_to_mask: Optional[List[str]] = None
    ...
    Raises:
        ...
        KeyError: If required columns are missing from the input DataFrame.
    """
    # Initialize the category columns to mask if not provided
    if category_columns_to_mask is None:
    ...
    try:
        if menu_mp == 0:
            ...
            # Add to track columns for masking
            category_columns_to_mask.append(consumption_col)
        ...
        # Add to track columns for masking
        category_columns_to_mask.append(cost_col)
        ...
        if menu_mp != 0:
            ...
            if baseline_col in df.columns:
                ...
                # Add to track columns for masking
                category_columns_to_mask.extend([baseline_col, savings_col])
