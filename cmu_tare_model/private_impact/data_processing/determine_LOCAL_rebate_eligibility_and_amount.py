...
df['local_rebate_eligibility'] = df['local_rebate_eligibility'].astype(str).str.lower().map({
    'yes': True,
    'no': False,
    'nan': np.nan
})
...
AMI_LOW_CUTOFF, AMI_MODERATE_CUTOFF   = 0.80, 1.50
HEEHR_COVERAGE_LOW, HEEHR_COVERAGE_MOD = 1.00, 0.50



HEEHR_CAP_HEAT_PUMP                   = 8_000
HOMES_MIN_SAVINGS_FRAC                = 0.20
HOMES_TIER2_SAVINGS_FRAC              = 0.35
HOMES_CAP_TIER1, HOMES_CAP_TIER2      = 2_000, 4_000   # non-LMI (see A5)
HOMES_COVERAGE_NON_LMI                = 0.50

ELECTRIC_RESISTANCE_BASELINE = {"electricity"}
REBATE_ELIGIBLE_HEATING_MPS  = {MP4}     # ENERGY STAR; gates BOTH programs (A4). Leave hardcoded.
REBATE_NONE, REBATE_HEEHR, REBATE_HOMES = "None", "HEEHR", "HOMES"


def determine_rebate(home, menu_mp, policy_scenario) -> tuple[float, str]:
    """Return (rebate_amount, rebate_eligibility). One program per home (A3)."""
    if not home.valid_heating or menu_mp not in home.scheduled_packages:
        return (NaN, REBATE_NONE)

    # ENERGY STAR gate, applied to both programs (A4). MP3 is never rebated.
    if menu_mp not in REBATE_ELIGIBLE_HEATING_MPS:
        return (0.0, REBATE_NONE)

    # Fuel gate -- the June 2026 change. Applies to both programs.
    # PLACEHOLDER (A2): dual-fuel installs ARE eligible under the guidance but
    # have no TARE measure package.
    if policy_scenario == POLICY_JUNE_2026:
        if home.baseline_heating_fuel not in ELECTRIC_RESISTANCE_BASELINE:
            return (0.0, REBATE_NONE)

    # PLACEHOLDER (A1): weatherization prerequisite not enforced.
    # PLACEHOLDER (A6): state-level funding caps not applied.

    pct_ami = home.household_income / home.area_median_income

    if pct_ami <= AMI_MODERATE_CUTOFF:                          # HEEHR
        coverage = HEEHR_COVERAGE_LOW if pct_ami <= AMI_LOW_CUTOFF else HEEHR_COVERAGE_MOD
        return (min(HEEHR_CAP_HEAT_PUMP, coverage * home.heat_pump_project_cost), REBATE_HEEHR)

    # >150% AMI -> HOMES, savings-based. All such homes are non-LMI (A5).
    savings = home.modeled_savings_frac      # whole-home site energy, modeled pathway (A7)
    if savings < HOMES_MIN_SAVINGS_FRAC:
        return (0.0, REBATE_NONE)
    cap = HOMES_CAP_TIER2 if savings >= HOMES_TIER2_SAVINGS_FRAC else HOMES_CAP_TIER1
    return (min(cap, HOMES_COVERAGE_NON_LMI * home.total_project_cost), REBATE_HOMES)
...
def calculate_percent_AMI(df_results_IRA: pd.DataFrame, random_seed: int = 42) -> pd.DataFrame:
    ...
    # Detailed conditional logic for income level 
