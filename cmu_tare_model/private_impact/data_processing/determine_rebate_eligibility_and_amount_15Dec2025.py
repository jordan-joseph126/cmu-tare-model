...
def calculate_rebate(
    ...
    try:
        ...
        install_cost_col = f'mp{menu_mp}_{category}_upgrade_installed_cost_{percentile}'
        ...
        enclosure_col = f'mp{menu_mp}_enclosure_upgrade_installed_cost'
        if enclosure_col in df_results_IRA.columns and menu_mp in [9, 10]:
            if enclosure_col in row and not pd.isna(row[enclosure_col]):
                weatherization_project_coverage = round(row[enclosure_col] * coverage_rate, 2)
                df_results_IRA.at[row.name, weatherization_rebate_col] = min(weatherization_project_coverage, max_weatherization_rebate_amount)
            else:
                df_results_IRA.at[row.name, weatherization_rebate_col] = 0.00
        ...
        df_results_IRA.at[row.name, rebate_col] = 0.00
        if menu_mp in [9, 10] and weatherization_rebate_col in df_results_IRA.columns:
            df_results_IRA.at[row.name, weatherization_rebate_col] = 0.00
...
def calculate_rebateIRA(
    ...
    if menu_mp in [9, 10]:
        weatherization_rebate_col = 'weatherization_rebate_amount'
        df_copy[weatherization_rebate_col] = 0.0
        
        # Track weatherization column under the category
        category_columns_to_mask.append(weatherization_rebate_col)
    ...
    def apply_rebate(row):
        ...
        if income_designation == 'Low-Income':
            calculate_rebate(df_copy, row, category, menu_mp, 1.00, percentile)

        elif income_designation == 'Moderate-Income':
            calculate_rebate(df_copy, row, category, menu_mp, 0.50, percentile)

        else:
            df_copy.at[row.name, rebate_col] = 0.00
            if menu_mp in [9, 10] and weatherization_rebate_col in df_copy.columns:
                df_copy.at[row.name, weatherization_rebate_col] = 0.00
