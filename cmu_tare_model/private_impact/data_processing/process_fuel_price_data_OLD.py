...
def project_future_prices(row, factor_dict, policy_scenario):
    ...
    Args:
        row (pd.Series): A pandas Series representing a row of data with location, fuel type, and 2022 price information.
        factor_dict (dict): A dictionary mapping (location, fuel type, policy scenario) to projection factors.
        policy_scenario (str): The policy scenario to use for projections (e.g., 'No Inflation Reduction Act').

    Returns:
        pd.Series: A pandas Series containing projected prices for each year (2022-2050).
    ...
    # If no factors are available, return an empty Series
    if not projection_factors:
        return pd.Series()

    # Calculate future prices based on projection factors
    future_prices = {}
    for year in range(2022, 2051):
        if year in projection_factors:
            factor = projection_factors[year]
            future_prices[f'{year}_fuelPrice_perkWh'] = price_2022 * factor
