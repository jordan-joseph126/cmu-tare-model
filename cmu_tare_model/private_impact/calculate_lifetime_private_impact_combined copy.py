...
def calculate_private_npv(
    ...
    for category, lifetime in EQUIPMENT_SPECS.items():
        ...
        if category == 'heating':
            # For heating, we need to track additional columns for combined heating and cooling replacement costs
            all_columns_to_mask[category].extend([
                f'mp{menu_mp}_heating_and_cooling_replacement_installed_cost_{percentile}'
            ])
...
def calculate_capital_costs(
    ...
    if policy_scenario == 'No Inflation Reduction Act':
        if category == 'heating':
            ...
            replacement_cost_heating = df_copy[f'mp{menu_mp}_{category}_replacement_installed_cost_{percentile}'].fillna(0)
            replacement_cost_heating_and_cooling = df_copy[f'mp{menu_mp}_heating_and_cooling_replacement_installed_cost_{percentile}'].fillna(0)

            net_capital_cost = total_capital_cost - replacement_cost_heating
            net_capital_cost_combined = total_capital_cost - replacement_cost_heating_and_cooling
    ...
    if category == 'heating':
        net_capital_cost_combined_masked = pd.Series(np.nan, index=df_copy.index)
        net_capital_cost_combined_masked.loc[valid_mask] = net_capital_cost_combined.loc[valid_mask]
        return total_capital_cost_masked, net_capital_cost_masked, net_capital_cost_combined_masked
    else:
