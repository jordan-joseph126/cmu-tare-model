...
def create_multiIndex_adoption_df(
    ...
    print(f"\nDEBUGGING FOR: {category} (mp{menu_mp}, {rcm_model}, {cr_function})")
    print(f"Looking for columns:")
    print(f"  - {adoption_cols[0]}")
    print(f"  - {adoption_cols[1]}")
    
    # Print the first few columns of the DataFrame to see patterns
    print(f"\nSample of actual columns in DataFrame ({len(df.columns)} total):")
    adoption_related = []
    for col in df.columns:
        if 'adoption' in str(col):
            adoption_related.append(col)
    
    # Print first 5 adoption-related columns, or message if none found
    if adoption_related:
        print(f"Found {len(adoption_related)} adoption-related columns:")
        for i, col in enumerate(adoption_related[:5]):
            print(f"  {i+1}. {col}")
        if len(adoption_related) > 5:
            print(f"  ... and {len(adoption_related)-5} more")
    else:
        print("  No adoption-related columns found!")
        
        # Print a few random columns to see what's available
    ...
    if not all(col in df.columns for col in adoption_cols):
        ...
        if all(col in df.columns for col in old_cols):
            ...
            # Alternative pattern might be without RCM/CR details
            alt_cols = [
            ...
            if all(col in df.columns for col in alt_cols):
                adoption_cols = alt_cols
                print(f"Using alternative column names for {category}")
            else:
                # Check if adoption columns exist for this category but with a different pattern
                category_adoption = []
                pattern = f'_{category}_adoption'
                for col in df.columns:
                    if pattern in str(col):
                        category_adoption.append(col)
                
                if category_adoption:
                    print(f"\nFound adoption columns for {category} with different pattern:")
                    for i, col in enumerate(category_adoption[:5]):
                        print(f"  {i+1}. {col}")
                    
                    # Try to identify a pattern
                    print("\nUnable to find expected column pattern. Please ensure columns follow the pattern:")
                    print(f"  'preIRA_mp{{menu_mp}}_{category}_adoption_{{scc}}_{{rcm_model}}_{{cr_function}}'")
                    print(f"  or backup pattern: 'preIRA_mp{{menu_mp}}_{category}_adoption_lrmer'")
    ...
    try:
        ...
        income_col = 'lowModerateIncome_designation'
        
        # Check if these groupby columns exist
        if fuel_col not in df.columns:
            print(f"Error: Required fuel column '{fuel_col}' not found!")
            return pd.DataFrame()
            
        if income_col not in df.columns:
            print(f"Error: Required income column '{income_col}' not found!")
            return pd.DataFrame()
        
        print(f"\nGrouping by: {fuel_col} and {income_col}")
        print(f"Using adoption columns: {adoption_cols}")
