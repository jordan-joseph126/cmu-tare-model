...
def calculate_total_npv_values(
    ...
    public_npv_col: str,
    output_col_names: Tuple[str, str]
) -> pd.DataFrame:
    """
    Calculates total NPV values by summing private and public NPV components.
    
    This is a core helper function that eliminates code duplication across all adoption
    analysis functions. It handles the common pattern of:
    1. Validating that homes have both private and public NPV data
    2. Calculating total NPV = private NPV + public NPV
    3. Only updating valid homes with calculated values
    
    Args:
        df_new_columns: DataFrame to store calculated values.
        df_copy: Source DataFrame with NPV data.
        valid_mask: Boolean mask identifying valid homes for analysis.
        private_npv_cols: Tuple of (lessWTP_col, moreWTP_col) column names.
        public_npv_col: Name of public NPV column.
        output_col_names: Tuple of (lessWTP_output, moreWTP_output) column names.
        
    Returns:
        Updated df_new_columns with total NPV calculations.
        
    Raises:
        KeyError: If required columns don't exist.
        ValueError: If column access returns DataFrame instead of Series.
    """
    lessWTP_private_col, moreWTP_private_col = private_npv_cols
    lessWTP_output_col, moreWTP_output_col = output_col_names
    ...
    try:
        # Calculate lessWTP total NPV
        valid_less_rows = (valid_mask & 
                          df_copy[lessWTP_private_col].notna() & 
                          df_copy[public_npv_col].notna())
        
        if valid_less_rows.any():
            df_new_columns.loc[valid_less_rows, lessWTP_output_col] = (
                df_copy.loc[valid_less_rows, lessWTP_private_col] + 
                df_copy.loc[valid_less_rows, public_npv_col]
            )
        
        # Calculate moreWTP total NPV
        valid_more_rows = (valid_mask & 
                          df_copy[moreWTP_private_col].notna() & 
                          df_copy[public_npv_col].notna())
        
        if valid_more_rows.any():
            df_new_columns.loc[valid_more_rows, moreWTP_output_col] = (
        ...
        if "DataFrame" in str(e):
            # Provide specific guidance for DataFrame vs Series issue
...
def classify_adoption_tiers(
    ...
    upgrade_column: str,
    private_npv_cols: Tuple[str, str],
    total_npv_cols: Tuple[str, str],
    adoption_col: str
) -> pd.DataFrame:
    """
    Classifies homes into adoption tiers based on economic feasibility.
    ...
    """
    lessWTP_private_col, moreWTP_private_col = private_npv_cols
    lessWTP_total_col, moreWTP_total_col = total_npv_cols
    
    # Initialize all homes with default values
    df_new_columns[adoption_col] = 'N/A: Invalid Baseline Fuel/Tech'
    
    # Set default for valid homes
...
def determine_public_impacts(
    ...
    """
    # Initialize all homes with default value
    df_new_columns.loc[valid_mask, impact_col] = 'N/A: Already Upgraded!'
...
def fix_duplicate_columns(df: pd.DataFrame, verbose: bool = False) -> pd.DataFrame:
    """
    Fix duplicate column names by renaming duplicates with suffixes.
    ...
    for col in df_fixed.columns:
        if col in column_counter:
            ...
            new_col_name = f"{col}_duplicate_{column_counter[col]}"
            new_columns.append(new_col_name)
            if verbose:
                print(f"  Renamed duplicate '{col}' → '{new_col_name}'")
        else:
    ...
    if verbose:
        print(f"✅ Fixed duplicate columns. DataFrame now has {len(df_fixed.columns)} unique column names")
            ...
            try:
                ...
                for scc in SCC_ASSUMPTIONS:
                    try:
                        ...
                        # Calculate total NPV values using helper
                        df_new_columns = calculate_total_npv_values(
                            ...
                            output_col_names=(new_col_names['lessWTP_total_npv'], new_col_names['moreWTP_total_npv'])
                        )
                        
                        # Classify adoption tiers using helper
                        df_new_columns = classify_adoption_tiers(
                            df_new_columns, df_copy, valid_mask, upgrade_column,
                            private_npv_cols=(lessWTP_private_npv_col, moreWTP_private_npv_col),
                            total_npv_cols=(new_col_names['lessWTP_total_npv'], new_col_names['moreWTP_total_npv']),
                        ...
                        df_new_columns = determine_public_impacts(
                            df_new_columns, df_copy, valid_mask,
                            public_npv_col, new_col_names['impact']
                        )
            ...
            for scc in scc_assumptions:
                try:
                    ...
                    df_new_columns = calculate_total_npv_values(
                        ...
                        output_col_names=(climate_col_names['lessWTP_total_npv'], climate_col_names['moreWTP_total_npv'])
                    )
                    ...
                    df_new_columns = classify_adoption_tiers(
                        df_new_columns, df_copy, valid_mask, upgrade_column,
                        private_npv_cols=(lessWTP_private_npv_col, moreWTP_private_npv_col),
                        total_npv_cols=(climate_col_names['lessWTP_total_npv'], climate_col_names['moreWTP_total_npv']),
                        adoption_col=climate_col_names['adoption']
                    )
                    
                    # Determine public impacts using helper
                    df_new_columns = determine_public_impacts(
                        df_new_columns, df_copy, valid_mask,
                        climate_npv_col, climate_col_names['impact']
                    )
        ...
        try:
            ...
            try:
                ...
                df_new_columns = calculate_total_npv_values(
                    ...
                    output_col_names=(health_col_names['lessWTP_total_npv'], health_col_names['moreWTP_total_npv'])
                )
                ...
                df_new_columns = classify_adoption_tiers(
                    df_new_columns, df_copy, valid_mask, upgrade_column,
                    private_npv_cols=(lessWTP_private_npv_col, moreWTP_private_npv_col),
                    total_npv_cols=(health_col_names['lessWTP_total_npv'], health_col_names['moreWTP_total_npv']),
                    adoption_col=health_col_names['adoption']
                )
                
                # Determine public impacts using helper
                df_new_columns = determine_public_impacts(
                    df_new_columns, df_copy, valid_mask,
                    health_npv_col, health_col_names['impact']
                )
