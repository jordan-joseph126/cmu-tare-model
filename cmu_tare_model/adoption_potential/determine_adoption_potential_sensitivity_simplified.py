...
def adoption_decision(
    ...
    Unified function that handles combined, climate-only, and health-only NPV calculations
    in a single pass through the data. Simplified output for nation-level analysis.
    ...
    Returns:
        DataFrame with adoption tier, public impact classifications, and total NPV values
        for combined, climate-only, and health-only scenarios.
    ...
    try:
        ...
        for category, upgrade_column in UPGRADE_COLUMNS.items():
            try:
                ...
                # Calculate health-only NPV once per category (doesn't vary by SCC)
                health_npv_calculated = False
                ...
                for scc in SCC_ASSUMPTIONS:
                    try:
                        ...
                        # Enhanced column names including climate/health-only NPV
                        new_col_names = {
                        ...
                        if missing_required:
                            if verbose:
                                print(f"Skipping {category}/{scc}: Missing columns {missing_required}")
                            continue
                        ...
                        # Also convert climate and health NPV columns if they exist
                        if climate_npv_col in df_copy.columns:
                            df_copy[climate_npv_col] = pd.to_numeric(df_copy[climate_npv_col], errors='coerce')
                        if health_npv_col in df_copy.columns:
                        ...
                        # ===== COMBINED TOTAL NPV (ORIGINAL LOGIC) =====
                        ...
                        # ===== HEALTH-ONLY TOTAL NPV (only calculate once per category) =====
                        if not health_npv_calculated and health_npv_col in df_copy.columns:
                            ...
                            health_npv_calculated = True
                        elif health_npv_calculated:
                            # Remove health NPV from tracking for subsequent SCC iterations
                            new_col_names.pop('total_npv_health', None)
                        
                        # Set defaults for adoption and impact
