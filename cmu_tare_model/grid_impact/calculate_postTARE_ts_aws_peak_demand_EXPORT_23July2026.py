...
for mp in selected_mps:
    ...
    # npv_case is passed by keyword (the study base case) so a positional slip
    # cannot silently substitute another case.
    ...
    for cs in REMDB_COST_SCENARIO_KEYS:
        try:
            adoption_col = find_adoption_column(
                ...
                mp=mp,
                cost_scenario=cs,
                discount_rate_key=DISCOUNT_RATE_KEY,
                npv_case=BASE_CASE_NPV_CASE,
            )
            print(f"  [OK] Adoption column (cost_scenario='{cs}'): {adoption_col}")
            break
        except KeyError:
            continue
...
def plot_demand_panel(
    ...
    """
    # ── Font size constants ───────────────────────────────────────────────────
    ANNOTATION_FONTSIZE, AXIS_LABEL_FONTSIZE, TICK_FONTSIZE = 14, 14, 14
    ...
    # ── Annotations to the right of each dashed line ─────────────────────────
    # xytext x is offset +60 hours right of the peak hour so the box left edge
    # clears the dashed line; ha="left" anchors the box there.
    _x_offset = 180
    ax.annotate(
        f"Base peak\n{peak_mw_base:.1f} MW\n(hr {peak_hr_base})",
        xy=(peak_hr_base, 0.95),
        xycoords=("data", "axes fraction"),
        xytext=(peak_hr_base + _x_offset, 0.95),
        textcoords=("data", "axes fraction"),
        fontsize=ANNOTATION_FONTSIZE, color="tab:red",
        ha="left", va="top",
    ...
    ax.annotate(
        ...
        xycoords=("data", "axes fraction"),
        xytext=(peak_hr_scen + _x_offset, 0.95),
        textcoords=("data", "axes fraction"),
        fontsize=ANNOTATION_FONTSIZE, color="tab:blue",
        ha="left", va="top",
        bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="tab:blue", alpha=0.7),
    )

    # ── Labels ────────────────────────────────────────────────────────────────
    ax.set_xlabel("Hour of Year", fontsize=AXIS_LABEL_FONTSIZE)
    ax.set_ylabel("Demand (MW)", fontsize=AXIS_LABEL_FONTSIZE)
...
mp_labels = {3: "Standard ASHP (15 SEER1, 9 HSPF1)", 4: "High-Efficiency ASHP (24-29.3 SEER1, 13-14 HSPF1)"}
...
for row_idx, (scenario, scenario_label) in enumerate(zip(scenarios, scenario_labels)):
    for col_idx, mp in enumerate(selected_mps):
        ...
        ax.set_title(f"{mp_labels[mp]} | {scenario_label}", fontsize=14)

# plt.suptitle("Hourly Electricity Demand — Allegheny County, PA (AMY2018)", fontsize=15)
plt.tight_layout()
...
# %% [markdown]
# ---
# ## Step 7d: Task 3.4 — Heating Fuel Distribution: Allegheny County Adopters
# ---
# 
# Tabular comparison of baseline heating fuel breakdown across all 4 combinations:
# - **MP3 / MP4** × **Constrained (Tier 1+2)** / **100% adoption**
# 
# Each cell shows count and percentage of that fuel type within the scenario.
# Percentages sum to 100% per column.
# 
...
# STEP 7d: Task 3.4 — Heating Fuel Distribution: Allegheny County Adopters
# =============================================================================
# Compares baseline heating fuel breakdown across all 4 combinations:
#   - MP3 constrained (Tier 1+2) vs. MP3 100% adoption
#   - MP4 constrained (Tier 1+2) vs. MP4 100% adoption
# adopter_ids_by_mp already contains only Allegheny County IDs — used directly.
# =============================================================================
...
# ── Collect fuel counts and percentages for all 4 combinations ───────────────
_results = {}
for _mp in selected_mps:
    _df_tare = DATAFRAMES_BY_MP[_mp]['fixed_base']['inmap']
    for _scenario, _key in [("constrained", "constrained"), ("100pct", "all_filtered")]:
        ...
        assert abs(_pcts.sum() - 100) < 1e-6, \
            f"MP{_mp} {_scenario}: pct sum ≠ 100 ({_pcts.sum():.6f})"
        _results[(_mp, _scenario)] = {"n": len(_ids), "counts": _counts, "pcts": _pcts}

# ── Collect all fuel categories across all combinations ───────────────────────
_all_fuels = sorted(set().union(*[v["counts"].index for v in _results.values()]))

# ── Print table ───────────────────────────────────────────────────────────────
_col_width = 26
_fuel_width = 20

print(f"Allegheny County (FIPS {TEST_FIPS}) — Baseline Heating Fuel Distribution")
print("=" * (_fuel_width + _col_width * 4 + 3))

# Header rows
_headers = []
for _mp in selected_mps:
    _lbl = _mp_label_fuel.get(_mp, f"MP{_mp}")
    _n_con = _results[(_mp, "constrained")]["n"]
    _n_all = _results[(_mp, "100pct")]["n"]
...
for _fuel in _all_fuels:
    ...
    for _mp in selected_mps:
        for _scenario in ("constrained", "100pct"):
            _c = _results[(_mp, _scenario)]["counts"].get(_fuel, 0)
            _p = _results[(_mp, _scenario)]["pcts"].get(_fuel, 0.0)
...
for _mp in selected_mps:
    for _scenario in ("constrained", "100pct"):
        _n = _results[(_mp, _scenario)]["n"]
        print(f"  {f'{_n:,} (100.0%)':>{_col_width - 2}}", end="")
print()

print("\n✓ Verification: all pct columns sum to 100.00%")
